...
async function viewRequestDetail(id){
  ...
  c.innerHTML=pageHead('קריאה '+r.request_no, (r.stores?.name||''),
    `<div class="row" style="gap:8px"><button class="btn" id="intake-pdf">${icon('print',15)} אישור קליטה</button><a href="#/requests" class="btn ghost">${icon('back',16)} חזרה</a></div>`);
...
function waLink(r){
  ...
  const T={
    ...
    awaiting_approval:`שלום ${name}, לגבי הכלי ${tool} (קריאה ${no}) — נשמח לחזור אליך בנוגע להצעת מחיר / חלקים.`,
