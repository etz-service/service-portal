...
function renderShell(){
  ...
  const shell=el(`<div class="shell">
    ...
    <div class="main">
      <div class="topbar">
        <div class="row"><div class="logo">${LOGO}</div><b>פורטל שירות ותיקונים עץ האורן</b></div>
...
async function adminDash(){
  ...
  dataView(rows=>{
    ...
    c.appendChild(sw);
    const mine=rows.filter(r=>['awaiting_approval','at_supplier','inspection','in_repair'].includes(r.stage));
...
async function fetchRequests(){
  ...
  _reqInflight=(async()=>{
    const {data,error}=await sb.from('service_requests').select(REQ_SELECT)
...
function confirmDelete(){
  ...
  foot.querySelector('#d-ok').onclick=async()=>{
    ...
    const ids=[..._sel];
    const {error}=await sb.from('service_requests').update({is_archived:true}).in('id',ids);
    if(error){toast('שגיאה: '+error.message,'err');btn.disabled=false;btn.textContent='כן, מחק';return;}
...
function waLink(r,customText){
  ...
  const T={
    ...
    opened:`שלום ${name}, הכלי ${tool} (קריאה ${no}) אושר ונשלח לספק בקרוב.`,
...
/* שליחת אישור קליטה ללקוח בוואטסאפ — מעלה את האישור, יוצר קישור צפייה, ופותח וואטסאפ */
async function sendIntakeWhatsapp(r,btn){
  const orig=btn.innerHTML; btn.disabled=true; btn.innerHTML='<span class="spinner"></span>';
  try{
    const html=buildIntakeHTML(r);
    const path=`${r.store_id}/${r.id}/intake_${Date.now()}.html`;
    const {error:eU}=await sb.storage.from(STORAGE_BUCKET).upload(path,new Blob([html],{type:'text/html'}),{contentType:'text/html'});
    if(eU)throw eU;
    // שמירה כקובץ מצורף + קישור צפייה לשנה
    sb.from('attachments').insert({request_id:r.id,store_id:r.store_id,storage_path:path,file_name:`אישור קליטה ${r.request_no}.html`,mime_type:'text/html',uploaded_by:State.profile.id}).then(()=>{});
    const {data:signed,error:eS}=await sb.storage.from(STORAGE_BUCKET).createSignedUrl(path,60*60*24*365);
    if(eS)throw eS;
    const tool=((r.tools?.brand||'')+' '+(r.tools?.model||'')).trim();
    const msg=`שלום ${r.customers?.full_name||''}, קיבלנו את הכלי ${tool} (קריאה ${r.request_no}). מצורף אישור קליטה:\n${signed.signedUrl}`;
