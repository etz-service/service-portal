...
const STAGES = [
  ...
  ['awaiting_approval','ממתין לחלקים'],
  ['in_repair','בתיקון'],
...
const STAGE_SHORT = {
  ...
  awaiting_approval:'ממתין לחלקים', in_repair:'בתיקון', handled:'מוכן לחזרה',
...
const FLOW = {
  ...
  inspection:        {next:'in_repair',         by:'supplier', label:'אישור לתיקון'},
  awaiting_approval: {next:'in_repair',         by:'supplier', label:'אישור לתיקון'},
...
const NAV_ORDER=['#/dash','#/intake','#/dispatch','#/send','#/receive','#/new','#/requests','#/stores','#/users','#/reports'];
let _prevBase=null;
...
async function adminDash(){
  ...
  const stats=[
    ...
    ['ממתין לחלקים', count('awaiting_approval'),''],
  ...
  c.appendChild(el(`<div class="stats stagger">${stats.map(s=>`<div class="stat ${s[2]}"><div class="n tnum">${s[1]}</div><div class="l">${esc(s[0])}</div></div>`).join('')}</div>`));
