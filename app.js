...
const STAGES = [
  ...
  ['in_repair','בתיקון'],
  ['handled','טופל'],
  ['in_transit_to_store','בדרך לחנות'],
  ['received_at_store','התקבל בחנות'],
  ['ready_for_pickup','מוכן לאיסוף'],
  ['delivered_closed','נמסר ונסגר'],
];
...
function renderShell(){
  ...
  const who=esc(State.profile.full_name), role=isAdmin()?'מנהל מערכת':'מנהל סניף';
  $('#app').innerHTML='';
...
async function adminDash(){
  ...
  const stats=[
    ['קריאות פתוחות', rows.filter(r=>OPEN_STAGES.includes(r.stage)).length,'accent'],
    ['חדשות (נפתחו)', count('opened'),''],
    ['אצל הספק', count('at_supplier'),''],
    ['בבדיקה', count('inspection'),''],
    ['בתיקון', count('in_repair'),''],
    ['ממתין לחלקים/אישור', count('awaiting_approval'),''],
    ['מוכן להחזרה/איסוף', count('handled')+count('in_transit_to_store')+count('received_at_store')+count('ready_for_pickup'),''],
  ...
  const recent=await loadRequests({limit:8});
  c.appendChild(requestsTable(recent,true));
}
...
async function storeDash(){
  ...
  const stats=[
    ['חדשות', count('opened'),''],
    ['בטיפול', rows.filter(r=>['at_supplier','inspection','awaiting_approval','in_repair','handled'].includes(r.stage)).length,''],
    ['מוכן לאיסוף', count('ready_for_pickup'),'accent'],
    ['נסגרו', count('delivered_closed'),''],
  ];
  c.innerHTML=pageHead('לוח בקרה', State.profile.branch_id?'הקריאות של הסניף שלך':'הקריאות של החנות שלך',
    `<a href="#/new" class="btn primary">${icon('plus',16)} פתיחת קריאה חדשה</a>`);
  c.appendChild(el(`<div class="stats">${stats.map(s=>`<div class="stat ${s[2]}"><div class="n tnum">${s[1]}</div><div class="l">${esc(s[0])}</div></div>`).join('')}</div>`));
  const ready=rows.filter(r=>r.stage==='ready_for_pickup');
  if(ready.length){c.appendChild(el(`<div class="section-title">ממתין לאיסוף לקוח</div>`));c.appendChild(requestsTable(ready,false));}
  c.appendChild(el(`<div class="section-title">קריאות אחרונות</div>`));
  c.appendChild(requestsTable(rows.slice(0,10),false));
}

/* ============================================================
   טעינת קריאות (עם join)
   ============================================================ */
async function loadRequests(opts={}){
  let q=sb.from('service_requests')
    .select('*, customers(full_name,phone,city,street), tools(category,brand,model,serial_no,sku,warranty), stores(name), branches(name)')
...
async function requestsList(){
  ...
  const apply=()=>{
    ...
    holder.innerHTML=''; holder.appendChild(requestsTable(list,isAdmin()));
...
function requestsTable(list,adminCols){
  ...
  cols.forEach(col=>{
    ...
    if(col[0])th.onclick=()=>{if(_sortCol===col[0])_sortDir*=-1;else{_sortCol=col[0];_sortDir=1;}
      const holder=wrap.parentElement;holder.innerHTML='';holder.appendChild(requestsTable(list,adminCols));};
    tr.appendChild(th);
  });
  ...
  sorted.forEach(r=>{
    const row=el(`<tr>
      ...
      ${adminCols?`<td>${esc(r.stores?.name||'—')}${r.branches?.name?`<br><span class="small muted">${esc(r.branches.name)}</span>`:''}</td>`:''}
      ...
      <td><button class="btn sm">פתיחה</button></td>
    </tr>`);
    row.querySelector('button').onclick=()=>location.hash='#/requests/id/'+r.id;
...
function exportCSV(list){
  const head=['מספר קריאה','נפתחה','חנות','סניף','ממלא','לקוח','טלפון','סוג כלי','מותג','דגם','מספר סידורי','סטטוס','אחריות','ימים בטיפול','עדכון אחרון'];
  const rows=list.map(r=>[r.request_no,fmtDate(r.created_at),r.stores?.name,r.branches?.name,r.opened_by_name,
...
async function newRequestWizard(){
  ...
  let branches=[];
  if(isAdmin()){ const {data}=await sb.from('branches').select('id,name,store_id,stores(name)').eq('is_active',true); branches=data||[]; }
  else { const {data}=await sb.from('branches').select('id,name,store_id').eq('store_id',State.profile.store_id).eq('is_active',true); branches=data||[]; }
  const data={step:1};
  ...
  function render(){
    ...
    body.innerHTML='';
    if(data.step===1){
      const branchOpts=branches.map(b=>[b.id, (isAdmin()&&b.stores?b.stores.name+' · ':'')+b.name]);
      body.appendChild(el('<div class="section-title" style="margin-top:0">פרטי הסניף והעובד</div>'));
      body.appendChild(fieldset([
        {name:'branch_id',label:'סניף',type:'select',req:true,options:[['','בחר סניף...'],...branchOpts],value:data.branch_id},
        {name:'opened_by_name',label:'שם ממלא הקריאה',req:true,value:data.opened_by_name||State.profile.full_name},
      ]));
      ...
      const b=branches.find(x=>x.id===data.branch_id);
      body.appendChild(el(`<div class="section-title" style="margin-top:0">סיכום הקריאה</div>
        <dl class="kv">
          <dt>סניף</dt><dd>${esc(b?((isAdmin()&&b.stores?b.stores.name+' · ':'')+b.name):'—')}</dd>
  ...
  $('#wz-next').onclick=async()=>{
    if(data.step===1){ if(!validate(body,['branch_id','opened_by_name','full_name','phone']))return; save(); data.step=2; render(); }
    ...
    else { await submitRequest(data,branches,filesToUpload); }
  };
  render();
}

async function submitRequest(data,branches,files){
  ...
  const store_id=branch.store_id, branch_id=branch.id;
  try{
    const {data:cust,error:e1}=await sb.from('customers').insert({
      store_id,branch_id,full_name:data.full_name,phone:data.phone,phone_alt:data.phone_alt||null,
    ...
    const {data:req,error:e3}=await sb.from('service_requests').insert({
      store_id,branch_id,opened_by:State.profile.id,opened_by_name:data.opened_by_name,
...
async function viewRequestDetail(id){
  ...
  const {data:r,error}=await sb.from('service_requests')
    .select('*, customers(*), tools(*), stores(name), branches(name), repair_details(*)')
    .eq('id',id).single();
  ...
  c.innerHTML=pageHead('קריאה '+r.request_no, (r.stores?.name||'')+(r.branches?.name?' · '+r.branches.name:''),
  ...
  // פאנל פעולה: ניהול סטטוס/תיקון (אדמין) או מסירה (סניף)
  if(isAdmin()){
    side.appendChild(adminActionPanel(r,rd));
  } else {
    if(r.stage==='ready_for_pickup'){
      const p=el(`<div class="card pad"><div class="section-title" style="margin-top:0">מסירה ללקוח</div>
        <p class="small muted">הכלי מוכן לאיסוף. פתח את מסך המסירה והחתמת הלקוח.</p>
        <button class="btn primary block" id="deliver-btn">${icon('check',16)} מסירה והחתמה</button></div>`);
      p.querySelector('#deliver-btn').onclick=()=>deliveryFlow(r);
      side.appendChild(p);
    } else {
      side.appendChild(el(`<div class="card pad"><div class="section-title" style="margin-top:0">סטטוס נוכחי</div>${pill(r.stage)}
        <p class="small muted" style="margin-top:10px">הקריאה בטיפול הספק. תעודכן כאן בכל שינוי.</p></div>`));
    }
  }
...
function adminActionPanel(r,rd){
  const p=el(`<div class="card pad"><div class="section-title" style="margin-top:0">ניהול הקריאה</div>
    <div class="field"><label>שינוי סטטוס</label>
    ...
    <div class="field"><label>הערה לשינוי (נשמרת בהיסטוריה)</label><input class="input" id="stage-note" placeholder="לא חובה"></div>
    <button class="btn primary block" id="save-stage">${icon('check',16)} עדכון סטטוס</button>
    <div style="height:12px"></div>
    <button class="btn block" id="edit-repair">${icon('wrench',15)} עריכת פרטי בדיקה ותיקון</button>
  </div>`);
  const toggleResult=()=>p.querySelector('#result-wrap').classList.toggle('hidden',p.querySelector('#new-stage').value!=='handled');
  p.querySelector('#new-stage').onchange=toggleResult; toggleResult();
  p.querySelector('#save-stage').onclick=async()=>{
    const stage=p.querySelector('#new-stage').value, result=p.querySelector('#new-result').value||null, note=p.querySelector('#stage-note').value.trim();
    if(stage==='handled'&&!result){toast('בחר תוצאת טיפול','err');return;}
    const btn=p.querySelector('#save-stage');btn.disabled=true;btn.innerHTML='<span class="spinner"></span>';
    const patch={stage}; if(stage==='handled')patch.result=result;
    const {error}=await sb.from('service_requests').update(patch).eq('id',r.id);
    if(error){toast('שגיאה: '+error.message,'err');btn.disabled=false;btn.textContent='עדכון סטטוס';return;}
    await sb.from('status_history').insert({request_id:r.id,store_id:r.store_id,from_stage:r.stage,to_stage:stage,note:note||null,changed_by:State.profile.id});
    await sb.from('audit_log').insert({actor_id:State.profile.id,store_id:r.store_id,entity:'service_request',entity_id:r.id,action:'status_change',details:{from:r.stage,to:stage}});
    toast('הסטטוס עודכן','ok'); viewRequestDetail(r.id);
  };
  p.querySelector('#edit-repair').onclick=()=>editRepairModal(r,rd);
  return p;
}
...
function deliveryFlow(r){
  ...
  foot.querySelector('#d-save').onclick=async()=>{
    ...
    try{
      ...
      await sb.from('deliveries').insert({request_id:r.id,store_id:r.store_id,branch_id:r.branch_id,
...
async function storesView(){
  const c=$('#content');
  c.innerHTML=pageHead('חנויות וסניפים','ניהול החנויות, הסניפים והפרטים שלהן',
    `<button class="btn primary" id="add-store">${icon('plus',16)} הוספת חנות</button>`);
  const {data:stores}=await sb.from('stores').select('*, branches(id,name,is_active)').eq('is_archived',false).order('name');
  ...
  (stores||[]).forEach(s=>{
    const card=el(`<div class="card pad">
      <div class="row spread wrap">
        ...
        <div class="row"><button class="btn sm" data-edit>עריכה</button><button class="btn sm primary" data-branch>${icon('plus',14)} סניף</button></div>
      </div>
      <div class="row wrap" style="gap:8px;margin-top:12px">
        ${(s.branches||[]).map(b=>`<span class="chip">${icon('pin',13)} ${esc(b.name)}</span>`).join('')||'<span class="small muted">אין סניפים</span>'}
      </div></div>`);
    card.querySelector('[data-edit]').onclick=()=>storeModal(s);
    card.querySelector('[data-branch]').onclick=()=>branchModal(s);
...
function branchModal(store){
  const body=fieldset([
    {name:'name',label:'שם הסניף',req:true},
    {name:'phone',label:'טלפון',type:'tel'},
    {name:'address',label:'כתובת',full:true},
  ]);
  const foot=el('<div class="row"><button class="btn primary" id="b-save">הוספה</button><button class="btn ghost" id="b-cancel">ביטול</button></div>');
  const m=openModal('הוספת סניף · '+store.name,body,foot);
  foot.querySelector('#b-cancel').onclick=m.close;
  foot.querySelector('#b-save').onclick=async()=>{
    if(!validate(body,['name']))return;const f=readForm(body);
    const {error}=await sb.from('branches').insert({...f,store_id:store.id});
    if(error){toast('שגיאה: '+error.message,'err');return;}
    toast('הסניף נוסף','ok');m.close();storesView();
  };
}
...
async function usersView(){
  ...
  const [{data:profiles},{data:stores},{data:branches}]=await Promise.all([
    sb.from('profiles').select('*, stores(name), branches(name)').order('created_at'),
    sb.from('stores').select('id,name').eq('is_archived',false).order('name'),
    sb.from('branches').select('id,name,store_id'),
  ]);
  const holder=el('<div class="tbl-wrap"><table class="tbl"><thead><tr><th>שם</th><th>תפקיד</th><th>חנות / סניף</th><th>סטטוס</th><th>כניסה אחרונה</th><th></th></tr></thead><tbody></tbody></table></div>');
  const tb=holder.querySelector('tbody');
  (profiles||[]).forEach(p=>{
    const row=el(`<tr>
      <td><b>${esc(p.full_name)}</b></td>
      <td>${p.role==='super_admin'?'מנהל מערכת':'מנהל סניף'}</td>
      <td>${esc([p.stores?.name,p.branches?.name].filter(Boolean).join(' · ')||'—')}</td>
  ...
  c.appendChild(holder);
  $('#add-user').onclick=()=>userModal(stores||[],branches||[]);
}
function userModal(stores,branches){
  const body=el('<div></div>');
  body.appendChild(fieldset([
    ...
    {name:'store_id',label:'חנות',type:'select',req:true,options:[['','בחר...'],...stores.map(s=>[s.id,s.name])]},
    {name:'branch_id',label:'סניף (רשות)',type:'select',options:[['','כל החנות']]},
  ]));
  const storeSel=body.querySelector('[name="store_id"]'), branchSel=body.querySelector('[name="branch_id"]');
  storeSel.onchange=()=>{const opts=branches.filter(b=>b.store_id===storeSel.value);
    branchSel.innerHTML='<option value="">כל החנות</option>'+opts.map(b=>`<option value="${b.id}">${esc(b.name)}</option>`).join('');};
  ...
  foot.querySelector('#u-save').onclick=async()=>{
    ...
    const res=await callAdmin('create_user',{full_name:f.full_name,username:f.username.toLowerCase(),password:f.password,store_id:f.store_id,branch_id:f.branch_id||null});
