...
function requestsTable(list,adminCols,showQuick,selectable){
  ...
  sorted.forEach(r=>{
    ...
    actCell.appendChild(el('<span class="act-spacer"></span>'));
    const openBtn=el('<button class="btn sm open-btn">פתיחה</button>'); openBtn.onclick=()=>location.hash='#/requests/id/'+r.id; actCell.appendChild(openBtn);
...
async function sendIntakeWhatsapp(r,btn){
  ...
  try{
    ...
    const {error:eU}=await sb.storage.from(STORAGE_BUCKET).upload(path,blob,{contentType:'image/png'});
    if(eU)throw eU;
    sb.from('attachments').insert({request_id:r.id,store_id:r.store_id,storage_path:path,file_name:`אישור קליטה ${r.request_no}.png`,mime_type:'image/png',uploaded_by:State.profile.id}).then(()=>{});
